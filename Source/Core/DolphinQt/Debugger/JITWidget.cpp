...
{
  const auto [near_free, near_frag] = m_system.GetJitInterface().GetNearMemoryInfo();
  const auto [far_free, far_frag] = m_system.GetJitInterface().GetFarMemoryInfo();
  m_status_bar->showMessage(
      QStringLiteral("Free memory: %1 near (%2% fragmented) %3 far (%4% fragmented)")
          .arg(QString::fromStdString(UICommon::FormatSize(near_free, 2)))
          .arg(near_frag * 100.0, 3, 'f', 2)
          .arg(QString::fromStdString(UICommon::FormatSize(far_free, 2)))
          .arg(far_frag * 100.0, 3, 'f', 2));
}
...
void JITWidget::CrossDisassemble(const JitBlock& block)
{
  ...
  const std::size_t host_instruction_count =
...
void JITWidget::TableEraseBlocks()
{
  ...
  // std::greater<QModelIndex> doesn't work. Do a backwards compare instead.
  std::sort(index_list.begin(), index_list.end(),
            [](const QModelIndex& lhs, const QModelIndex& rhs) { return rhs < lhs; });
