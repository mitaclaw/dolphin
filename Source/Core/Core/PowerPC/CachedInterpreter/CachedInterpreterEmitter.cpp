...
s32 CachedInterpreterCodeBlock::PoisonCallback(const PoisonOperands& operands)
{
...
void CachedInterpreterCodeBlock::PoisonMemory()
{
  ...
  std::fill(reinterpret_cast<AnyCallback*>(region),
            reinterpret_cast<AnyCallback*>(region + region_size),
            reinterpret_cast<AnyCallback>(&PoisonCallback));
}
