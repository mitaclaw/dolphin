...
void JITWidget::CrossDisassemble(const JitBlock& block)
{
  ...
  const auto host_near_instruction_count = jit_interface.DisasmNearCode(block, stream);
  m_host_near_asm_widget->setPlainText(QString::fromStdString(std::move(stream).str()));

  const auto host_far_instruction_count = jit_interface.DisasmFarCode(block, stream);
