...
std::pair<std::size_t, double> JitInterface::GetNearMemoryInfo() const
{
  if (m_jit)
    return m_jit->GetNearMemoryInfo();
  return {0, 1.0};
}

std::pair<std::size_t, double> JitInterface::GetFarMemoryInfo() const
{
  if (m_jit)
    return m_jit->GetFarMemoryInfo();
  return {0, 1.0};
}
